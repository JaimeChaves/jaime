...
class Usuario {
	...
	function Cadastrar($email,$usuario, $senha){
		...
		$consulta = mysqli_query(Conexao::$connection, "SELECT * FROM usuario where usuario='$usuario' ");
		$conConsulta = mysqli_num_rows($consulta);
		
		if($conConsulta == 0){
			mysqli_query(Conexao::$connection, "INSERT INTO usuario VALUES('','$this->email', '$this->usuario', '$this->senha', '".date("Y/m/d H:m:s")."') ");
	...
	$this->Email = AntiSql($email);
	$this->Usuario = AntiSql($usuario);
	$sql = mysql_query(Conexao::$connection, "SELECT * FROM usuario WHERE email = '$this->Email'");
	$cont = mysql_num_rows($sql);
		...
		$senha2 = md5($senha1);
		mysqli_query(Conexao::$connection, "UPDATE usuario SET senha = '$senha2' WHERE email='$this->Email' ");
	...
	$sql = mysqli_query(Conexao::$connection, "SELECT * FROM usuario WHERE usuario='$_COOKIE[usuario]'");
